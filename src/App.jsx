...
function App() {
  ...
  const navItems = ["Works", "Certifications", "Contact"];
  ...
  return (
    <div className="min-h-screen max-w-[1440px] mx-auto bg-[#fdfdfd] text-black font-sans selection:bg-black selection:text-white border-x border-black/10 relative">
        ...
        <nav className="hidden md:flex gap-6">
          {navItems.map((item) => (
            <a
              ...
              className="text-xs font-bold uppercase tracking-widest hover:line-through transition-all"
            >
              {item}
            </a>
          ))}
        ...
        <div className="flex flex-col md:flex-row gap-12 items-end">
          <div className="flex-1 animate-reveal">
            ...
            <div className="mt-8 flex gap-4 animate-reveal delay-200">
              <a
                href="#contact"
                className="bg-black text-white px-8 py-4 font-bold uppercase tracking-wide hover:bg-gray-800 transition-colors border border-black"
              >
                Ready to Work
              </a>
              <a
                ...
                download
                className="bg-white text-black px-8 py-4 font-bold uppercase tracking-wide hover:bg-gray-100 transition-colors border border-black flex items-center gap-2"
              >
        ...
        <div className="flex w-full">
          <div className="flex animate-marquee min-w-full shrink-0 items-center justify-around">
            {skills.map((skill, i) => (
              <span
                key={i}
                className="text-xl md:text-3xl font-black uppercase tracking-widest px-8 flex items-center gap-8"
              >
                {skill}{" "}
        ...
        <div>
          {projects.map((project) => (
            <div
              ...
              className="group relative border-b border-black cursor-pointer overflow-hidden transition-colors duration-300 hover:bg-black hover:text-white"
            >
              <div className="px-6 py-16 md:py-20 flex flex-col md:flex-row justify-between items-start md:items-center relative z-10">
                <div className="flex flex-col">
                  <span className="text-xs font-mono mb-2 opacity-50 group-hover:text-gray-400">
                    0{project.id} / {project.category}
                  </span>
                  <h3 className="text-4xl md:text-6xl font-black uppercase tracking-tighter transform transition-transform duration-300 group-hover:translate-x-4">
                    {project.title}
                  </h3>
                </div>
      ...
      <section
        id="contact"
        className="grid grid-cols-1 md:grid-cols-2 border-b border-black bg-gray-50"
      >
        <div className="p-12 md:p-24 border-b md:border-b-0 md:border-r border-black flex flex-col justify-between">
          ...
          <div className="flex flex-col gap-2">
            ...
            <a
              ...
              className="text-xl md:text-2xl font-bold underline decoration-2 underline-offset-4 hover:bg-black hover:text-white transition-colors inline-block w-max px-1"
            >
        ...
        <div className="p-12 md:p-24 flex flex-col justify-center gap-8 bg-white">
          <div className="space-y-6">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center justify-between border-b border-black pb-4 group hover:pl-4 transition-all"
            >
              <span className="text-2xl font-bold uppercase">LinkedIn</span>
            ...
            <a
              ...
              className="flex items-center justify-between border-b border-black pb-4 group hover:pl-4 transition-all"
            >
              <span className="text-2xl font-bold uppercase">GitHub</span>
              <span className="group-hover:-rotate-45 transition-transform duration-300">
                →
              </span>
            </a>
      ...
      <footer className="py-8 text-center text-xs font-mono uppercase tracking-widest text-gray-400">
        © {new Date().getFullYear()} Rizal M. Nur — Designed with Precision.
      </footer>
      ...
      {selectedItem && (
        ...
        >
          <div
            className="w-full max-w-6xl bg-white border border-gray-200 shadow-2xl flex flex-col max-h-[95vh] overflow-hidden animate-reveal"
          ...
          >
                ...
                <>
                  ...
                  {selectedItem.images && selectedItem.images.length > 1 && (
                    <div className="absolute inset-0 flex justify-between items-center px-4 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={prevImage}
                        className="w-12 h-12 bg-white/90 rounded-full flex items-center justify-center hover:scale-110 transition-transform shadow-lg"
                      >
                        ←
                      </button>
                      <button
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {selectedItem.tech.map((t, i) => (
                    <span
                      key={i}
                      className="border border-black px-3 py-1 text-xs font-bold uppercase tracking-wider hover:bg-black hover:text-white transition-colors"
                    >
                      {t}
                    </span>
                  ))}
            ...
            <button
              onClick={closeModal}
              className="absolute top-4 right-4 bg-white w-10 h-10 rounded-full flex items-center justify-center font-bold shadow-lg hover:rotate-90 transition-transform"
            >
              ×
            </button>
          </div>
